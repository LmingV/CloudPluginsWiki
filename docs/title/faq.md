...
2. 显示变量的插件需要定时刷新（例如 TAB 插件）；刷新间隔越长，动画跳帧越明显。见[动画与刷新](./placeholders.md#动画与刷新)。
   **刷新间隔刚好等于一轮动画的长度（或它的整数倍）时，每次都会取到同一帧，看起来完全不动。** 例如 10 帧 × 10 fps = 一轮 1 秒，而头顶插件每 1 秒刷新一次（UnlimitedNameTags 的 `taskInterval: 20`）。把刷新间隔改短即可，10 fps 的称号建议 `taskInterval: 2`（0.1 秒）。
